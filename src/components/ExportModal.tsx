  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
        {/* Header */}
        <div className="bg-orange-600 px-8 py-6 text-white flex items-center justify-between">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <select 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-orange-500/20"
                >
              ...
              {[
                ...
                `}>
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <span className={`text-xs font-bold ${config[item.id as keyof ExportConfig] ? 'text-orange-600' : 'text-slate-500'}`}>
                      {item.label}
                    </span>
                    {config[item.id as keyof ExportConfig] && <CheckCircle2 size={14} className="text-orange-500" />}
                  </div>
                  ...
                  return (
                    <button 
                      ...
                      className={`
                        ...
                        ${active ? 'border-orange-500 bg-orange-50 text-orange-600' : 'border-slate-100 bg-slate-50 text-slate-500 hover:border-slate-200'}
                      `}
                    >
                      {city.label}
                    </button>
        ...
        <div className="p-8 bg-slate-50 flex items-center justify-end gap-4 border-t border-slate-100">
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-orange-600 hover:bg-orange-700 text-white px-8 py-3 rounded-2xl font-bold text-sm shadow-lg shadow-orange-200 transition-all active:scale-95"
          >
            開始生成報告
