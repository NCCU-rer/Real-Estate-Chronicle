    ...
    return (
      <div className="bg-white/95 backdrop-blur-md p-3 border border-slate-200 rounded-xl shadow-2xl text-xs z-50 ring-1 ring-slate-100 min-w-40">
        <p className="font-bold text-slate-700 mb-2 border-b border-slate-100 pb-1.5 flex items-center gap-2">
          <span className="w-1 h-3 bg-[#B7791F] rounded-full"></span>
          {label}
        </p>
    ...
    <div className="bg-white/80 backdrop-blur-sm p-3 text-xs h-16 border-b border-slate-100 sticky top-0 z-10">
      <div className="flex items-center justify-between mb-2 h-5">
         <p className="font-bold text-slate-700 truncate flex items-center gap-2">
           {label ? (
             <>
               <span className="w-1.5 h-1.5 bg-[#B7791F] rounded-full animate-pulse"></span>
               {label}
             </>
           ) : (
           ...
           <ResponsiveContainer width="100%" height="100%">
            ...
            >
              <Brush 
                dataKey="quarter" 
                height={20} 
                stroke="#B7791F"     // ★ 選取框邊框：亮橘色
                travellerWidth={20} 
                fill="#B7791F20"       // ★ 選取框背景：淺橘色
